...
class TFTemperatureLogitsWarper(TFLogitsWarper):
    r"""[`TFLogitsWarper`] for temperature (exponential scaling output probability distribution).
    ...
    def __init__(self, temperature: float) -> None:
        if not isinstance(temperature, float) or not (temperature > 0):
            msg = f"`temperature` has to be a strictly positive float, but is {temperature}"
            raise ValueError(msg)
...
class TFTopKLogitsWarper(TFLogitsWarper):
    r"""[`TFLogitsWarper`] that performs top-k, i.e. restricting to the k highest probability elements.
    ...
    def __call__(self, input_ids: tf.Tensor, scores: tf.Tensor, cur_len: int) -> tf.Tensor:
        ...
        return tf.where(indices_to_remove, self.filter_value, scores)
...
class TFTopPLogitsWarper(TFLogitsWarper):
    ...
    def __call__(self, input_ids: tf.Tensor, scores: tf.Tensor, cur_len: int) -> tf.Tensor:
        ...
        return tf.scatter_nd(scatter_indices, topk_next_scores, shape=topk_next_scores.shape)


class TFMinLengthLogitsProcessor(TFLogitsProcessor):
    r"""[`TFLogitsProcessor`] enforcing a min-length by setting EOS probability to 0.
    ...
    def __init__(self, min_length: int, eos_token_id: int) -> None:
        if not isinstance(min_length, int) or min_length < 0:
            msg = f"`min_length` has to be a positive integer, but is {min_length}"
            raise ValueError(msg)

        if not isinstance(eos_token_id, int) or eos_token_id < 0:
            msg = f"`eos_token_id` has to be a positive integer, but is {eos_token_id}"
            raise ValueError(msg)
    ...
    def _apply_eos_token_mask(self, scores: tf.Tensor) -> tf.Tensor:
        ...
        return tf.where(eos_token_id_mask, float("-inf"), scores)
...
class TFRepetitionPenaltyLogitsProcessor(TFLogitsProcessor):
    r"""[`TFLogitsProcessor`] enforcing an exponential penalty on repeated sequences.
    ...
    def _create_score_penalties(self, input_ids: tf.Tensor, logits: tf.Tensor) -> tf.Tensor:
        ...
        return tf.tensor_scatter_nd_update(token_penalties, indices=indexable_prev_input_ids, updates=tf.reshape(logit_penalties, [-1]))

    def __call__(self, input_ids: tf.Tensor, scores: tf.Tensor, cur_len: int) -> tf.Tensor:
        score_penalties = self._create_score_penalties(input_ids[:, :cur_len], scores)

        return tf.math.multiply(scores, score_penalties)
...
class TFNoBadWordsLogitsProcessor(TFLogitsProcessor):
    ...
    def _calc_row_banned_bad_tokens(self, row_input_ids: tf.Tensor) -> tf.Tensor:
        def _tokens_match(bad_word_seq_number):
            ...
            def _match_found():
                ...
                return tf.cond(
                    tf.math.reduce_all(tf.math.equal(row_input_ids[-compare_len:], self.bad_word_seqs_ids[bad_word_seq_number, :compare_len])),
                    lambda: tf.ones((), dtype=tf.bool),
                    lambda: tf.zeros((), dtype=tf.bool),
                )
        ...
        return self.seq_forbidden_tokens[match_mask]
    ...
    def __call__(self, input_ids: tf.Tensor, scores: tf.Tensor, cur_len: int) -> tf.Tensor:
        ...
        def _get_row_updated_score(row_inputs: tuple[tf.Tensor]) -> tf.Tensor:
            ...
            return tf.where(banned_tokens_mask, -float("inf"), row_score)

        return tf.map_fn(_get_row_updated_score, (input_ids, scores), fn_output_signature=tf.float32)


class TFNoRepeatNGramLogitsProcessor(TFLogitsProcessor):
    r"""[`TFLogitsProcessor`] that enforces no repetition of n-grams. See
    ...
    def calc_banned_ngram_tokens(self, input_ids, num_hypos, cur_len):
        ...
        for idx in range(num_hypos):
            ...
            for ngram in zip(*[gen_tokens[i:] for i in range(self.ngram_size)], strict=False):
    ...
    def __call__(self, input_ids: tf.Tensor, scores: tf.Tensor, cur_len: int) -> tf.Tensor:
        ...
        if not tf.executing_eagerly():
            msg = "TFNoRepeatNGramLogitsProcessor is only implemented for eager execution."
            raise NotImplementedError(msg)
        ...
        return tf.where(tf.convert_to_tensor(banned_tokens_indices_mask, dtype=tf.bool), -float("inf"), scores)


class TFForcedBOSTokenLogitsProcessor(TFLogitsProcessor):
    r"""[`TFLogitsProcessor`] that enforces the specified token as the first generated token.
    ...
    def __init__(self, bos_token_id: int) -> None:
        if bos_token_id < 0:
            msg = f"The forced bos token id  must be a non-negative integer, got {bos_token_id}"
            raise ValueError(msg)
...
class TFForcedEOSTokenLogitsProcessor(TFLogitsProcessor):
    r"""[`TFLogitsProcessor`] that enforces the specified token as the last generated token when `max_length` is reached.
    ...
    def __init__(self, max_length: int, eos_token_id: int) -> None:
        ...
        if eos_token_id < 0:
            msg = f"The forced eos token id must be a non-negative integer, got {eos_token_id}"
            raise ValueError(msg)
        self.eos_token_id = eos_token_id
...
class TFSuppressTokensAtBeginLogitsProcessor(TFLogitsProcessor):
    r"""[`TFSuppressTokensAtBeginLogitsProcessor`] suppresses a list of tokens as soon as the `generate` function starts
...
class TFForceTokensLogitsProcessor(TFLogitsProcessor):
    ...
    def __call__(self, input_ids: tf.Tensor, scores: tf.Tensor, cur_len: int) -> tf.Tensor:
        def _force_token(generation_idx):
            ...
            return tf.tensor_scatter_nd_update(new_scores, indices, updates)
