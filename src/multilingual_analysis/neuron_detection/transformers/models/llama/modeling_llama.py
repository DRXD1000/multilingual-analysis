...
class LlamaMLP(nn.Module):
    ...
    def forward(self, x, early_layers):
        if self.config.pretraining_tp > 1:
            ...
            real_output = self.down_proj(self.act_fn(self.gate_proj(x)) * self.up_proj(x))
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: int | None = None) -> None:
        super().__init__()
        self.config = config
    ...
    def shared_forward(
        ...
        **kwargs,
        ...
        attn_output_temp = attn_output.reshape(bsz, q_len, self.hidden_size)
        ...
        return (
            ...
            attn_weights,
            past_key_value,
            attn_weights_temp.squeeze().tolist(),
            attn_weights_temp.squeeze().tolist(),
            attn_output_temp,
            torch.sum(torch.abs(attn_output_o), dim=1).squeeze().tolist(),
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        attn_output, attn_weights, past_key_value_real, query_score, key_score, attn_output_temp, o_score = self.shared_forward(
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states, hidden_score_fwd_up, hidden_score_fwd_down = self.mlp(hidden_states, early_layers)
        hidden_states = residual + hidden_states
...
class LlamaPreTrainedModel(PreTrainedModel):
    ...
    def _setup_cache(self, cache_cls, max_batch_size, max_cache_len: int | None = None) -> None:
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            early_layers_flag = idx in early_exit_layers
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def prepare_inputs_for_generation(self, input_ids, past_key_values=None, attention_mask=None, inputs_embeds=None, **kwargs):
        ...
        if self.generation_config.cache_implementation == "static":
            ...
            past_length = 0 if cache_position is None else cache_position[-1] + 1
            input_ids = input_ids[:, past_length:]
