...
class FlaxGenerationMixin:
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(self, input_ids, params, model_kwargs):
        encoder_kwargs = {argument: value for argument, value in model_kwargs.items() if not (argument.startswith(("decoder_", "cross_attn")))}
    ...
    def generate(
        ...
        **kwargs,
        ...
        if generation_config.decoder_start_token_id is None and self.config.is_encoder_decoder:
            msg = "`decoder_start_token_id` has to be defined for encoder-decoder generation."
            raise ValueError(msg)
        ...
        if (
            ...
            and not trace
            and (generation_config.pad_token_id is not None and jnp.sum(input_ids[:, -1] == generation_config.pad_token_id) > 0)
        ):
            logger.warning(
        ...
        if not generation_config.do_sample and generation_config.num_beams == 1:
            ...
            msg = "`Beam sampling is currently not implemented."
            raise NotImplementedError(msg)
    ...
    def _beam_search(
        ...
        def beam_search_cond_fn(state):
            ...
            worst_finished_score = jnp.where(state.is_sent_finished, jnp.min(state.scores, axis=1, keepdims=True), np.array(-1.0e7))
        ...
        state = self._run_loop_in_debug(beam_search_cond_fn, beam_search_body_fn, state) if not trace else lax.while_loop(beam_search_cond_fn, beam_search_body_fn, state)
