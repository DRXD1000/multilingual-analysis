...
class Constraint(ABC):
    ...
    def test(self) -> None:
        ...
        while not completed:
            ...
            if not self.does_advance(advance):
                msg = "Custom Constraint is not defined correctly. self.does_advance(self.advance()) must be true."
                raise Exception(msg)
            ...
            if counter > 10000:
                msg = "update() does not fulfill the constraint."
                raise Exception(msg)

        if self.remaining() != 0:
            msg = "Custom Constraint is not defined correctly."
            raise Exception(msg)

    @abstractmethod
    def advance(self) -> Never:
    ...
    @abstractmethod
    def does_advance(self, token_id: int) -> Never:
        """Reads in a token and returns whether it creates progress."""
        msg = f"{self.__class__} is an abstract class. Only classes inheriting this class can be called."
        raise NotImplementedError(msg)

    @abstractmethod
    def update(self, token_id: int) -> Never:
    ...
    @abstractmethod
    def remaining(self) -> Never:
        """Returns the number of remaining steps of `advance()` in order to complete this constraint."""
        msg = f"{self.__class__} is an abstract class. Only classes inheriting this class can be called."
        raise NotImplementedError(msg)

    @abstractmethod
    def copy(self, stateful=False) -> Never:
...
class PhrasalConstraint(Constraint):
    r"""[`Constraint`] enforcing that an ordered sequence of tokens is included in the output.
    ...
    def __init__(self, token_ids: list[int]) -> None:
        ...
        if not isinstance(token_ids, list) or len(token_ids) == 0:
            msg = f"`token_ids` has to be a non-empty list, but is {token_ids}."
            raise ValueError(msg)
        if any((not isinstance(token_id, int) or token_id < 0) for token_id in token_ids):
            msg = f"Each list in `token_ids` has to be a list of positive integers, but is {token_ids}."
            raise ValueError(msg)
    ...
    def does_advance(self, token_id: int):
        if not isinstance(token_id, int):
            msg = f"`token_id` has to be an `int`, but is {token_id} of type {type(token_id)}"
            raise TypeError(msg)
...
class DisjunctiveTrie:
    def __init__(self, nested_token_ids: list[list[int]], no_subsets=True) -> None:
        r"""A helper class that builds a trie with the words represented in `nested_token_ids`."""
        self.max_height = max([len(one) for one in nested_token_ids])

        root = {}
        for token_ids in nested_token_ids:
            level = root
            for _tidx, token_id in enumerate(token_ids):
        ...
        if no_subsets and self.has_subsets(root, nested_token_ids):
            msg = "Each list in `nested_token_ids` can't be a complete subset of another list, but is" f" {nested_token_ids}."
            raise ValueError(msg)
...
class DisjunctiveConstraint(Constraint):
    r"""A special [`Constraint`] that is fulfilled by fulfilling just one of several constraints.
    ...
    def __init__(self, nested_token_ids: list[list[int]]) -> None:
        ...
        if not isinstance(nested_token_ids, list) or len(nested_token_ids) == 0:
            msg = f"`nested_token_ids` has to be a non-empty list, but is {nested_token_ids}."
            raise ValueError(msg)
        if any(not isinstance(token_ids, list) for token_ids in nested_token_ids):
            msg = f"`nested_token_ids` has to be a list of lists, but is {nested_token_ids}."
            raise ValueError(msg)
        if any(any((not isinstance(token_id, int) or token_id < 0) for token_id in token_ids) for token_ids in nested_token_ids):
            msg = f"Each list in `nested_token_ids` has to be a list of positive integers, but is {nested_token_ids}."
            raise ValueError(msg)
    ...
    def does_advance(self, token_id: int):
        if not isinstance(token_id, int):
            msg = f"`token_id` is supposed to be type `int`, but is {token_id} of type {type(token_id)}"
            raise TypeError(msg)
...
class ConstraintListState:
    r"""A class for beam scorers to track its progress through a list of constraints.
    ...
    def add(self, token_id: int):
        if not isinstance(token_id, int):
            msg = f"`token_id` should be an `int`, but is `{token_id}`."
            raise TypeError(msg)
        ...
        if self.inprogress_constraint is not None:
            ...
            for cidx, pending_constraint in enumerate(self.pending_constraints):
                if pending_constraint.does_advance(token_id):
                    ...
                    if not stepped:
                        msg = "`constraint.update(token_id)` is not yielding incremental progress, " "even though `constraint.does_advance(token_id)` is true."
                        raise Exception(msg)
